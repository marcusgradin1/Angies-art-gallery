(function(){
  ...
  function addProductTools(){
    ...
    if(!document.querySelector('.ux-room-preview')){
      ...
      let roomZoom=1,roomX=0,roomY=0,roomDragging=false,startX=0,startY=0;const frame=room.querySelector('.ux-room');const scene=room.querySelector('.ux-room__scene');const applyRoomZoom=()=>{const maxX=Math.max(0,(frame.clientWidth*(roomZoom-1))/2),maxY=Math.max(0,(frame.clientHeight*(roomZoom-1))/2);roomX=Math.max(-maxX,Math.min(maxX,roomX));roomY=Math.max(-maxY,Math.min(maxY,roomY));scene.style.transform=`translate3d(${roomX}px,${roomY}px,0) scale(${roomZoom})`;frame.classList.toggle('is-zoomed',roomZoom>1);frame.classList.toggle('is-dragging',roomDragging);room.querySelectorAll('[data-room-zoom]').forEach(button=>{button.disabled=(button.dataset.roomZoom==='out'&&roomZoom<=1)||(button.dataset.roomZoom==='in'&&roomZoom>=2.2)})};
      ...
      room.addEventListener('pointerdown',e=>{if(e.target.closest('button')||roomZoom<=1)return;e.preventDefault();roomDragging=true;startX=e.clientX-roomX;startY=e.clientY-roomY;room.setPointerCapture(e.pointerId);applyRoomZoom()});
