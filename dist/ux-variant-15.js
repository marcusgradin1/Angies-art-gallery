(function(){
  ...
  function addProductTools(){
    ...
    if(!document.querySelector('.ux-room-preview')){
      ...
      let roomZoom=1;const scene=room.querySelector('.ux-room__scene');const applyRoomZoom=()=>{scene.style.transform=`scale(${roomZoom})`;room.querySelectorAll('[data-room-zoom]').forEach(button=>{button.disabled=(button.dataset.roomZoom==='out'&&roomZoom<=1)||(button.dataset.roomZoom==='in'&&roomZoom>=2.2)})};
